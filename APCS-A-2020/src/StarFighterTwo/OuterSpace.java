...
public class OuterSpace extends Canvas implements KeyListener, Runnable {
	...
	public void paint(Graphics window) {
		...
		graphToBack.drawString("Score: " + score, getWidth() - 80, getHeight() - 40);
		...
		for (Boolean b : shoot) {
			Ammo bullet = bullets.get(shoot.indexOf(b));
			if (b) {
		...
		for (Alien alien : horde) {
			...
			for (Ammo ammo : bullets) {
				if (ammo.getX() >= alien.getX() && ammo.getX() <= alien.getX() + alien.getWidth()
						&& ammo.getY() <= alien.getY() + alien.getHeight() && ammo.getY() >= alien.getY()) {
					shoot.set(bullets.indexOf(ammo), false);
					ammo.setPos(-100, -100);
					alien.setY(-40);
					score++;
					graphToBack.setColor(Color.BLACK);
					graphToBack.fillRect(720, 460, 30, 30);
				}
			}
			if (alien.getY() > 640) {
				alien.setY(-40);
		...
		for (Ammo ammo : bullets) {
			if (ammo.getY() < 0 || ammo.getY() > 600 || ammo.getX() < 0 || ammo.getX() > 600) {
				shoot.set(bullets.indexOf(ammo), false);
	...
	public void keyPressed(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_LEFT) {
			...
			for (Ammo a : bullets) {
				a.setDirection("LEFT");
				a.setPos(ship.getX() - ship.getWidth() / 2, ship.getY() + ship.getHeight() / 2);
			}
			resetShoot();
		}
		if (e.getKeyCode() == KeyEvent.VK_RIGHT) {
			...
			for (Ammo a : bullets) {
				...
				a.setPos(ship.getX() + ship.getWidth(), ship.getY() + ship.getHeight() / 2);
			}
			resetShoot();
		}
